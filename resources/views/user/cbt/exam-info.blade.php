        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-body text-center">
                    ...
                    <h2 class="m-3">{{ $session->exam->title }}</h2>
                    ...
                    @if($session->exam->description)
                        <p class="m-4">{{ $session->exam->description }}</p>
                    @endif

                    <div class="row justify-content-center m-4">
                        <div class="col-md-3">
                            <div class="bg-light rounded p-3">
                                <h3 class="text-primary m-0">{{ $session->exam->questions->count() }}</h3>
                                <small class="text-muted">Soal</small>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="bg-light rounded p-3">
                                <h3 class="text-info m-0">{{ $session->exam->duration_minutes }}</h3>
                                <small class="text-muted">Menit</small>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="bg-light rounded p-3">
                                <h3 class="text-warning m-0">{{ $session->exam->passing_score }}%</h3>
